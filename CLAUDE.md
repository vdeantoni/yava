...
`src/lib/utils.ts` exports named epsilons: `MIN_SLICE_DISTANCE` (0.5s minimum segment length), `MIN_FADE_DURATION` (0.1s, the shortest fade worth keeping), `FLUSH_TOLERANCE` (0.01s, treats boundaries as touching so segments can be joined), `PLAYBACK_TOLERANCE` (0.05s), `SEEK_TOLERANCE` (0.01s, below which a seek is a no-op and gets skipped), `RESTART_TOLERANCE` (0.1s). Reuse them along with `findSegmentAt`, `sliceIndexAt` and `snapToNearestSegmentBoundary` instead of inlining new comparisons. `sliceIndexAt` in particular is shared between the store action and the toolbar button, so what the button offers cannot drift from what the action does.
...
- Keep `cursorCurrent` out of anything expensive. It changes at pointer rate while scrubbing and a few times a second during playback. `Playhead` is a separate component for exactly this reason: it is the only part of the timeline whose position depends on the playhead, and the track around it renders every segment, handle and up to ~150 tick marks. `TimelineToolbar` derives each button's enabled and pressed state inside its selector for the same reason, so a playhead that moves without flipping a button costs nothing.
