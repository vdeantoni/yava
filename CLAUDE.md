...
- Keep `cursorCurrent` out of anything expensive. It changes at pointer rate while scrubbing and a few times a second during playback, and a 20-move scrub now re-renders `Playhead` and nothing else. Three things buy that: `Playhead` is its own component subscribing to `cursorCurrent` alone, reading segments with `useAppStore.getState()` in its pointer handler; `VideoPlayer` does not subscribe to it at all, driving its seek and its fade overlay from a `useAppStore.subscribe` because neither is rendered; and `TimelineToolbar` derives each button's enabled and pressed state inside its selector, so a playhead that moves without flipping a button costs nothing.
- `VideoTimeline` holds its tick and mark row in a `useMemo`. A segment drag re-renders the track on every pointer move, and those ~150 nodes move only with the scale, so handing React the same element lets it skip the subtree.
