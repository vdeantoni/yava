...
- Keep `cursorCurrent` out of anything expensive. It changes at pointer rate while scrubbing and a few times a second during playback. `Playhead` is a separate component for exactly this reason: it is the only part of the timeline whose position depends on the playhead, and the track around it renders every segment, handle and up to ~150 tick marks. It subscribes to `cursorCurrent` alone and reads the segments with `useAppStore.getState()` inside its pointer handler, so a segment edit does not re-render it either. `TimelineToolbar` derives each button's enabled and pressed state inside its selector for the same reason, so a playhead that moves without flipping a button costs nothing.
