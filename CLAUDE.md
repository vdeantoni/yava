...
Nothing reachable from script substitutes for that tap, and the restriction is per element, so a tap on the player does not reach the four off-screen extractors in `VideoThumbnails`. **The extractors have no fix.** They key on `loadedmetadata` rather than `loadeddata`, which an element that stopped never fires, so they at least reach their own seeks, and they skip the capture below readyState 2 rather than caching a blank frame. Whether a seek is enough to restart a stopped element is unverified. The next thing to try is `play()` on the extractors, which they can do without a gesture because they are already muted and inline.
