...
Four states end up here and `src/lib/media-notice.ts` holds every message, the state names, and the rules that tell them apart. Each state has a constructor there returning a whole `MediaNotice`, so no component ever pairs a message with a detail line or picks a tone. If the element raises an error, `describeMediaError` turns the code into one; Chromium fires that with a null `MediaError` for a codec it cannot use, so a missing code is read as an unsupported codec rather than something generic.
...
The fourth is not a failure. iOS Safari clamps `preload` to metadata whatever the attribute says, and reads no further until playback is requested by a **user gesture**: WebKit lifts the restriction in `prepareToPlay()`, which only `play()` calls. The element stops at readyState 1 with the network idle and paints nothing, so `describeBlankPicture` recognises `metadata`/`idle` and asks for the tap instead of blaming the decoder. Everything else works meanwhile, which is why the editor bootstraps on metadata. This is also why a large file stops and a short recording does not: a few megabytes arrive whole inside the metadata read, so the element never has to be asked.

Nothing reachable from script substitutes for that tap, and the restriction is per element, so a tap on the player does not reach the four off-screen extractors in `VideoThumbnails`. They ask for themselves instead: each one is muted and inline, which is the one case iOS permits playback without a gesture, so each calls `play()` when its metadata lands and pauses again on its first frame. That means they have to be **in the document**, since iOS refuses playback to an element with no renderer. They sit in a 1px transparent host inside the timeline, not `display: none`, which would take the renderer away again. They also key on `loadedmetadata` rather than `loadeddata`, which an element that stopped never fires, and skip the capture below readyState 2 rather than caching a blank frame.

Those extractors are why the page has several `video` elements. Each carries `data-thumbnail-extractor`, and `playerVideo` in `tests/e2e/helpers.ts` selects the player by excluding them.
